...
class AcousticRepairTests(unittest.TestCase):
    ...
    def test_repair_is_accepted_only_when_review_count_drops(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            acoustic.write_text(json.dumps({
                "model": "fixture",
    ...
    def test_repair_rejects_legacy_acoustic_profile(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            audio = root / "chapter.mp3"
            acoustic = root / "acoustic.json"
            ...
            aligned = root / "aligned.json"
            audio.write_bytes(b"audio fixture")
            analysis.write_text(json.dumps([{"id": "s-1", "text": "anchor"}]), encoding="utf-8")
            acoustic.write_text(json.dumps({
                "model": "fixture",
                "words": [{"word": "anchor", "start": 1, "end": 2}],
            }), encoding="utf-8")
            align_sentences_with_audio(acoustic, analysis, aligned)

            result = repair_acoustic_gaps(audio, acoustic, analysis, aligned)

        self.assertEqual(result["status"], "stale-acoustic-profile")
